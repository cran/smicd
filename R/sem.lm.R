...
semLm <- function(formula, data, classes, burnin = 40, samples = 200, trafo = "None",
  ...
  if (trafo=="bc") {lambda.est <- lambda.lm.est(formula = formula, data = data, burnin = burnin,
  ...
  parameter <- list(coef = resultcoef, sigmae = result_sigmae, r2 = result_r2,
                    adjr2 = result_r2adj)
  ...
  if (bootstrap.se == TRUE) {result_se <- standardErrorLM.est(formula = o.formula,
                                                               data = o.data,
                                                               classes = o.classes,
                                                               burnin = burnin,
                                                               samples = samples,
  ...
  est <- list(pseudo.y = resulty, coef = parameter.final$coef, sigmae = parameter.final$sigmae,
